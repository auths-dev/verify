...
| `auths-version` | Auths CLI version to **pin** (e.g. `0.1.1`). Required unless `auths` is on `PATH`; the action never resolves `latest` and fails closed without a verifiable `.sha256` | Yes (unless on PATH) | `''` |
...
- uses: auths-dev/verify@v1
  with:
    auths-version: "0.1.1"   # pin the CLI (required on clean runners)
```
